...
<div class="max-w-3xl mx-auto px-4 py-8">
	...
	<form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" class="space-y-6">
		...
		<div>
			...
			<input id="title" type="text" name="title" value="{{ old('title', $post->title) }}" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
		</div>

		<div>
			<label for="description" class="mb-1 block text-sm font-medium text-gray-700">Description</label>
			<textarea id="description" name="description" rows="10" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $post->description) }}</textarea>
		</div>

		@if ($post->image)
			<div>
				<label class="mb-1 block text-sm font-medium text-gray-700">Current Image</label>
				<img src="{{ Storage::url($post->image) }}" alt="{{ $post->title }}" class="h-32 w-32 rounded object-cover" />
			</div>
		@endif
		...
		<div class="flex items-center justify-end gap-3">
			...
			<button type="submit" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2">Update</button>
		</div>
	</form>
</div>
